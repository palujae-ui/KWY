...
export default function ResearchList() {
  ...
  const chip = (active: boolean) =>
    `px-4 py-1.5 rounded-full border text-xs md:text-sm font-medium transition-all duration-200 ${
      active
      ...
      <div className="flex items-center justify-between pb-4">
        ...
        {(topic || type) && (
          <button
            ...
            className="text-xs font-medium text-blue-600 hover:text-blue-700 bg-blue-50 px-3 py-1 rounded-full border border-blue-200/60 transition-colors"
          >
            필터 초기화 ↺
          </button>
        )}
